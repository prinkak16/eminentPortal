class Api::V1::Vacancy::VacancyController < BaseApiController
  ...
  def mov_excel_download
    begin
      ...
      organizations = organizations.where(ministries: { id: params[:ministry].split(',') }) if params[:ministry].present?
      organizations = organizations.where(vacancies: { allotment_status: params[:position_status] }) if params[:position_status].present?
      organizations = organizations.where(vacancies: { country_state_id: params[:country_state_id].split(',') }) if params[:country_state_id].present?
      organizations = organizations.where(departments: { id: params[:department].split(',') }) if params[:department].present?
      organizations = organizations.where(id: params[:organization].split(',')) if params[:organization].present?
